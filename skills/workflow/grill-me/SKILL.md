...
Interview the user rigorously about a plan, idea, design, or decision until the important assumptions, tradeoffs, risks, and downstream consequences are explicit.
...
Do not use this skill for simple Q&A, code implementation, ordinary code review, or automatic documentation updates.
...
2. Use available evidence before asking. If code, docs, issues, specs, briefs, ADRs, or notes can answer a question, inspect them instead of making the user restate known facts.
...
9. Stop only when the critical branches are answered, explicitly deferred, or no longer material to the user's goal.
...
- If a durable artifact may be useful, propose the artifact and ask before writing it.

## Output

Default output is the conversation itself: one question, recommendation, answer, and follow-up at a time.

When wrapping up, provide a concise summary of:

- Decisions made.
- Assumptions confirmed.
- Risks and open questions that remain.
- Suggested next step, if one is obvious.
...
- [ ] Dependent decisions were handled in a coherent order.
- [ ] Recommendations were separated from user decisions.
- [ ] Important assumptions, alternatives, risks, and failure modes were surfaced.
- [ ] Code or documentation evidence was used when it could answer a question.
- [ ] No artifact, documentation update, tracker action, or implementation was performed without explicit user approval.
